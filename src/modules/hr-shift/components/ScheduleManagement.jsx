...
import { ATTENDANCE_OPTIONS, PAGE_SIZE, getErrorMessage, normalizePageResult } from "../utils/staffUtils";
...
export default function ScheduleManagement() {
	...
	const loadSchedules = useCallback(async () => {
		try {
			...
			const hasFilter = Boolean(dateFilter) || Boolean(employeeFilter) || Boolean(shiftFilter) || attendanceFilter !== "all";
			...
			const result = hasFilter
				? await workScheduleService.searchWorkSchedules({
						...
						shiftId: shiftFilter,
						attendanceStatus: attendanceFilter === "all" ? "" : attendanceFilter,
						page: page,
						size: PAGE_SIZE,
					})
			...
			const filtered = normalized.content.filter((item) => {
				if (dateFilter && String(item.workDate || item.date || "").slice(0, 10) !== dateFilter) {
					return false;
				}
				if (employeeFilter && String(item.employeeId || item.employee?.id || "") !== String(employeeFilter)) {
					return false;
				}
				if (shiftFilter && String(item.shiftId || item.shift?.id || "") !== String(shiftFilter)) {
					return false;
				}
	...
	const openModal = (schedule = null) => {
		...
		setForm({
			...
			manualAttendance:
				schedule == null ? "" : schedule?.isPresent === true ? "true" : schedule?.isPresent === false ? "false" : "",
	...
	const handleSave = async () => {
		try {
			...
			const scheduleKeyChanged =
				!editingSchedule ||
				String(editingSchedule?.employeeId || editingSchedule?.employee?.id || "") !== String(form.employeeId) ||
				String(editingSchedule?.shiftId || editingSchedule?.shift?.id || "") !== String(form.shiftId) ||
				String(editingSchedule?.workDate || editingSchedule?.date || "").slice(0, 10) !== form.workDate;
	...
	const isAttendanceLocked = Boolean(editingSchedule && (editingSchedule?.checkInTime || editingSchedule?.checkOutTime));
				...
				<Card.Body>
					<div className="row g-3 align-items-end">
						...
						<div className="col-md-4 col-lg-3">
							...
							<Form.Select value={employeeFilter} onChange={(event) => setEmployeeFilter(event.target.value)}>
						...
						<div className="col-md-4 col-lg-3">
							...
							<Form.Select value={shiftFilter} onChange={(event) => setShiftFilter(event.target.value)}>
						...
						<div className="col-md-4 col-lg-2">
							...
							<Form.Select value={attendanceFilter} onChange={(event) => setAttendanceFilter(event.target.value)}>
					...
					{editingSchedule && (
						<div>
							...
							<Form.Select
								...
								onChange={(event) => setForm((prev) => ({ ...prev, manualAttendance: event.target.value }))}
							>
							...
							<small className="text-muted d-block mt-1">
								Điểm danh bù sẽ được hiển thị khi đánh dấu đi làm thủ công mà không có giờ check-in.
							</small>
						</div>
					)}
					{isAttendanceLocked && (
						<small className="text-muted">
							Lịch đã có chấm công nên không thể đổi Nhân viên, Ca làm và Ngày làm.
						</small>
					)}
					<div>
